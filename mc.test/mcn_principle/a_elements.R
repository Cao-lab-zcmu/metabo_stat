...
labels <- lapply(c(ec., ep., "Class..."),
                 function(lab) {
                   zo(into(grectn(bgp_args = list(lty = "solid")),
                           gtextp(stringr::str_wrap(lab, 15), list(col = "white"))))
                 })
...
venn.text <- frame_col(c(x = 1, null = 1, y = 1),
                       list(x = gtextp("Class."), y = gtextp("Class."),
                            null = nullGrob()))
venn <- frame_row(c(tit = .2, null = .1, cont = 1),
...
p.upset <- ggplot(data, aes(x = Class)) +
  ...
  ggupset::theme_combmatrix(combmatrix.label.text =

...
gt_table <- pretty_table(dplyr::rename(export.dominant[, -ncol(export.dominant)], info = Classification),
                         ...
                         footnote = "Compounds listed in table were identified from Raw-Eucommia or Pro-Eucommia. These compounds were grouped by classes which selected manually. As compounds not only belong to one class and also belong to its parent classes, for this case, the compounds were preferentially considered for subtile classes.",
                         default = F) %>% 
  ## add footnote
  ## name
  ...
  tab_footnote(footnote = "All identified formulae were in adduct of '[M - H]-'.",
  ...
  tab_footnote(footnote = "The variation were difined as: ↑ or ↓ indicated increasing or decreasing of compound level while comparing Pro-Eucommia with Raw-Eucommia.  - indicated no apparent alteration (|log2(FC)| < 1); the number of arrow implied the magnitude of alteration, i.e., integer of |log2(FC)|. FC was defined as Pro-Eucommia compare with Raw-Eucommia.",
